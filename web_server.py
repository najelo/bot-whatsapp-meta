...
def procesar_verificacion_pago_bg(phone: str, image_bytes: bytes, monto_esperado: float):
    ...
    try:
        ...
        if exito:
            ...
            # 🔥 PARTE A: Enviar comprobante rechazado al chat de RECHAZOS
            enviar_alerta_telegram(
                monto=monto_esperado,
                telefono=phone,
                razon=f"Fallo en verificación: {respuesta_ia}",
                image_bytes=image_bytes,
                tipo_alerta="alerta"  # 👈 Dirección al canal de comprobantes
            )
        ...
        # 🔥 PARTE B: Enviar excepción de código al chat de ERRORES
        enviar_alerta_telegram(
            monto=0.0,
            telefono=phone,
            razon=f"Fallo crítico interno en el servidor: {str(e)}",
            image_bytes=image_bytes,
            tipo_alerta="error"  # 👈 Dirección al canal de errores
        )
